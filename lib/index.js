  ...
  ctx.tools.register(defineTool({
    ...
    parameters: {
      action: {
        ...
        description: "要执行的操作：trend=趋势统计（最近 10-1000 期，默认 1000）；predict=预测下一期（可指定预测数据范围 window）；generate=生成号码",
        enum: ["trend", "predict", "generate"]
      },
      window: { type: "integer", description: "数据范围（期数）：10-1000，默认 1000，常用 100/200/500/1000（trend/predict 各自独立使用，如预测基于最近 500 期则传 window=500）" },
      preset: {
        type: "string",
