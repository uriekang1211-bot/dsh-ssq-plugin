...
function buildPredict(draws, windowSize, presetKey) {
  ...
  const lines = [
    `【预测下一期】模型：${p.model}（基于最近 ${s.windowSize} 期，${first.issue} ~ ${last.issue}）`,
...
function buildEnsemble(draws, windowSize) {
  const s = core.computeStats(draws, windowSize);
    ...
    async execute(args) {
      try {
        ...
        if (args.action === "ensemble") {
          const r = buildEnsemble(src.draws, Math.max(10, Math.min(1000, args.window || 1000)));
          r.message += "\n" + srcNote(src);
          return r;
        }
