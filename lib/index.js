...
// 本地保存的历史基线：内置快照（最近 1000 期），在线源不可达时的兜底
const SNAPSHOT_DRAWS = core.normalize(BUNDLED_RAW);
...
async function tryFetch(source) {
  ...
  const draws = core.normalize(source.adapt ? source.adapt(json) : json);
  if (draws.length < 10) throw new Error("有效数据不足(" + draws.length + "期)");
  return draws;
}

/** 以内置快照为基线做增量更新：只拉最新若干期合并，数据始终是最近 1000 期 */
async function loadDraws() {
  if (cache.draws && Date.now() - cache.at < 30 * 60 * 1000) return cache;

  // ① 官方增量：拉最新 200 期 → 与快照合并
  try {
    const fresh = await tryFetch(OFFICIAL_INCR);
    const m = core.mergeDraws(SNAPSHOT_DRAWS, fresh);
    if (m.addedCount > 0) {
      if (!m.hasGap) {
        cache = { at: Date.now(), draws: m.draws, source: `官方接口增量（基线快照合并）` };
        return cache;
      }
      // 快照过期太久（缺口）→ 回退官方全量重建
      try {
        const full = await tryFetch(OFFICIAL_FULL);
        if (full.length > 0) {
          cache = { at: Date.now(), draws: full.slice(-1000), source: `官方接口全量重建` };
          return cache;
        }
      } catch (e) { /* 全量失败则用带缺口的合并结果 */ }
      cache = { at: Date.now(), draws: m.draws, source: `官方接口增量（快照有缺口）` };
      return cache;
    }
    // 快照已是最新
    cache = { at: Date.now(), draws: SNAPSHOT_DRAWS, source: `内置快照（已是最新）` };
    return cache;
  } catch (e) { /* 尝试镜像 */ }
  ...
  for (const s of MIRROR_SOURCES) {
    try {
      const fresh = await tryFetch(s);
      const m = core.mergeDraws(SNAPSHOT_DRAWS, fresh);
      cache = { at: Date.now(), draws: m.draws, source: `${s.name}增量合并` };
      return cache;
    } catch (e) { /* 尝试下一源 */ }
  }

  // ③ 内置快照兜底
  return { at: 0, draws: SNAPSHOT_DRAWS, source: `内置快照（在线源暂不可达）` };
}

/* ---------------- 结果构造 ---------------- */
function srcNote(src) {
  const first = src.draws[0], last = src.draws[src.draws.length - 1];
  return `（数据源：${src.source}｜当前 ${src.draws.length} 期 ${first.issue} ~ ${last.issue}，最新 ${last.issue} 期 ${last.date}）`;
}
...
function buildPredict(draws, windowSize, presetKey, ewmaHalf) {
  const s = core.computeStats(draws, windowSize, ewmaHalf);
  const p = core.predict(s, presetKey);
  const first = s.draws[0], last = s.draws[s.windowSize - 1];
  const halfNote = presetKey === "ewma" ? `（半衰期 ${ewmaHalf} 期）` : "";
  const lines = [
    `【预测下一期】模型：${p.model}${halfNote}（基于最近 ${s.windowSize} 期，${first.issue} ~ ${last.issue}）`,
  ...
  return {
    ok: true,
    message: lines.join("\n"),
    detail: JSON.stringify({ model: p.model, window: s.windowSize, first: first.issue, last: last.issue, rec: p.rec, red: p.red.map(redRow), blue: p.blue.map(x => ({ n: x.n, p: +x.p.toFixed(4), zFreq: +x.zFreq.toFixed(2), zRec: +x.zRec.toFixed(2), zGap: +x.zGap.toFixed(2) })) })
  };
}
...
function buildEnsemble(draws, windowSize, ewmaHalf) {
  const s = core.computeStats(draws, windowSize, ewmaHalf);
  const en = core.ensemble(s);
  const first = s.draws[0], last = s.draws[s.windowSize - 1];
  const lines = [
    `【集成投票】${en.presetKeys.length} 个模型各自推荐 Top6 红球 + Top1 蓝球，票数高者胜出（基于最近 ${s.windowSize} 期，${first.issue} ~ ${last.issue}）`,
    `推荐号码：红 ${en.rec.red.map(pad).join(" ")} ｜ 蓝 ${pad(en.rec.blue)}`,
    `蓝球备选（票数 Top3）：${en.rec.blueAlt.map(pad).join("、")}`,
    `红球得票 Top6：${en.red.slice(0, 6).map(x => pad(x.n) + "(" + x.votes + "票)").join("、")}`,
    `⚠️ 开奖为独立随机事件，预测仅作娱乐参考，请理性购彩。`
  ];
  return {
    ok: true,
    message: lines.join("\n"),
    detail: JSON.stringify({ model: en.model, window: s.windowSize, first: first.issue, last: last.issue, rec: en.rec, red: en.red.map(x => ({ n: x.n, votes: x.votes })), blue: en.blue.map(x => ({ n: x.n, votes: x.votes })) })
  };
}

function buildStructure(draws, windowSize) {
  const s = core.structureStats(draws, windowSize);
  const first = s.latest ? draws.slice(-s.windowSize)[0].issue : "—";
  const last = s.latest.issue;
  const fmt = list => list.slice(0, 3).map(x => `${x.key}(${(x.freq * 100).toFixed(1)}%)`).join("、");
  const top = { oddEven: s.oddEven[0].key, size: s.size[0].key, zone: s.zone[0].key };
  const combos = core.genStructureCombos(top, 5);
  const lines = [
    `【组合结构预测】基于最近 ${s.windowSize} 期（${first} ~ ${last}）`,
    `奇偶比 Top3：${fmt(s.oddEven)}`,
    `大小比 Top3：${fmt(s.size)}`,
    `三区间 Top3：${fmt(s.zone)}`,
    `和值区间 Top3：${fmt(s.sum)}`,
    `最可能结构：奇偶 ${top.oddEven} · 大小 ${top.size} · 区间 ${top.zone}`,
    `按该结构随机生成 5 注：`,
    ...combos.map((c, i) => `${i + 1}. 红 ${c.red.map(pad).join(" ")} ｜ 蓝 ${pad(c.blue)}`),
    `⚠️ 开奖为独立随机事件，结构统计仅供娱乐参考，请理性购彩。`
  ];
  return {
    ok: true,
    message: lines.join("\n"),
    detail: JSON.stringify({ window: s.windowSize, first, last, oddEven: s.oddEven, size: s.size, zone: s.zone, sum: s.sum, top, combos: combos.map(c => ({ red: c.red, blue: c.blue })) })
  };
}
...
export function apply(ctx) {
  ctx.tools.register(defineTool({
    name: "ssq",
    description: "双色球（中国福利彩票）分析助手，基于最近 1000 期开奖数据。action=trend：统计最近 N 期（默认 1000 期）每个红球 1-33 / 蓝球 1-16 号码的出现次数、频率、遗漏与热温冷状态；action=predict：基于频率+近期衰减+遗漏回补加权模型预测下一期号码概率并给出推荐，可通过 window 参数指定预测数据范围（如最近 100/200/500/1000 期）；action=generate：按胆拖规则（dan 胆码 1-5 个 + tuo 拖码 + blues 蓝球）或纯随机生成号码。开奖为独立随机事件，结果仅供娱乐参考。",
    timeoutMs: 30000,
    parameters: {
      action: {
        type: "string", required: true,
        description: "要执行的操作：trend=趋势统计（最近 10-1000 期，默认 1000）；predict=预测下一期（可指定预测数据范围 window 与模型 preset）；ensemble=集成投票（全部模型投票融合）；structure=组合结构预测（奇偶/大小/区间/和值分布）；generate=生成号码",
        enum: ["trend", "predict", "ensemble", "structure", "generate"]
      },
      window: { type: "integer", description: "数据范围（期数）：10-1000，默认 1000，常用 100/200/500/1000（各 action 独立使用，如预测基于最近 500 期则传 window=500）" },
      preset: {
        type: "string",
        description: "预测模型：balanced=均衡混合（默认）；cold=冷号回补；hot=热号延续；ewma=EWMA 近期加权；miss=遗漏均值回归；expect=期望偏差回补（predict 使用）",
        enum: ["balanced", "cold", "hot", "ewma", "miss", "expect"]
      },
      ewmaHalf: { type: "integer", description: "EWMA 信号半衰期（期）：10-100，默认 20，常用 20/50/100（仅 preset=ewma 时影响评分）" },
      mode: {
        type: "string",
    ...
    async execute(args) {
      try {
        ...
        if (args.action === "predict") {
          const r = buildPredict(src.draws, Math.max(10, Math.min(1000, args.window || 1000)), args.preset || "balanced", Math.max(10, Math.min(100, args.ewmaHalf || 20)));
          r.message += "\n" + srcNote(src);
          return r;
        }
        if (args.action === "ensemble") {
          const r = buildEnsemble(src.draws, Math.max(10, Math.min(1000, args.window || 1000)), Math.max(10, Math.min(100, args.ewmaHalf || 20)));
          r.message += "\n" + srcNote(src);
          return r;
        }
        if (args.action === "structure") {
          const r = buildStructure(src.draws, Math.max(10, Math.min(1000, args.window || 1000)));
          r.message += "\n" + srcNote(src);
          return r;
        }
