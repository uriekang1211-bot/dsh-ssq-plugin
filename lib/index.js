 ...
 * 注册一个模型可调用的动态工具 `ssq`：
 *   - action=trend    百期趋势统计（次数/频率/遗漏/热度）
 *   - action=predict  频率+近期+遗漏回补 加权模型预测下一期
 *   - action=generate 胆拖规则 / 纯随机生成号码
 * 数据优先在线拉取（Host 无浏览器 CORS 限制），失败时回退内置快照。
 * 开奖为独立随机事件，结果仅供娱乐参考。
 * ============================================================ */
...
const DATA_SOURCES = [
  {
    name: "官方福彩接口",
    url: "https://www.cwl.gov.cn/cwl_admin/front/cwlkj/search/kjxx/findDrawNotice?name=ssq&issueCount=120",
    headers: {
...
export function apply(ctx) {
  ctx.tools.register(defineTool({
    name: "ssq",
    description: "双色球（中国福利彩票）分析助手。action=trend：统计最近 N 期（默认 100 期）每个红球 1-33 / 蓝球 1-16 号码的出现次数、频率、遗漏与热温冷状态；action=predict：基于频率+近期衰减+遗漏回补加权模型预测下一期号码概率并给出推荐；action=generate：按胆拖规则（dan 胆码 1-5 个 + tuo 拖码 + blues 蓝球）或纯随机生成号码。开奖为独立随机事件，结果仅供娱乐参考。",
    timeoutMs: 30000,
    parameters: {
      action: {
        type: "string", required: true,
        description: "要执行的操作：trend=百期趋势统计；predict=预测下一期；generate=生成号码",
        enum: ["trend", "predict", "generate"]
      },
      window: { type: "integer", description: "统计窗口期数，10-100，默认 100（trend/predict 使用）" },
      preset: {
        type: "string",
    ...
    async execute(args) {
      try {
        ...
        if (args.action === "trend") {
          const r = buildTrend(src.draws, Math.max(10, Math.min(100, args.window || 100)));
          r.message += "\n" + srcNote(src);
          return r;
        }
        if (args.action === "predict") {
          const r = buildPredict(src.draws, Math.max(10, Math.min(100, args.window || 100)), args.preset || "balanced");
          r.message += "\n" + srcNote(src);
          return r;
        }
