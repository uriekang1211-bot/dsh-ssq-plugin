 ...
 * @param draws - 时间正序的开奖数组
 * @param windowSize - 统计窗口期数
 * @param ewmaHalf - EWMA 信号半衰期（期），默认 20，可传 50/100 等（仅 preset=ewma 时显著影响评分）
 */
export function computeStats(draws, windowSize, ewmaHalf = 20) {
  ...
  const analyze = (arr, hit, isRed) => {
    ...
    // EWMA 信号：半衰期 ewmaHalf 期的指数加权出现频率（z 标准化），供「EWMA 近期加权」模型使用
