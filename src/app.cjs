...
/* ---- 开奖号码查询：期号选择器（有数据的全部期号可选，最新在前） ---- */
function refreshIssueSelect() {
  const sel = $("issueSelect");
  if (!sel) return;
  const prev = sel.value;
  sel.innerHTML = "";
  for (let i = DRAW_DATA.length - 1; i >= 0; i--) {
    const d = DRAW_DATA[i];
    sel.appendChild(el("option", { value: d.issue, text: d.issue + " 期" }));
  }
...
function renderIssueResult() {
  const sel = $("issueSelect"), box = $("issueResult"), dateEl = $("issueDate");
  if (!sel || !box || !DRAW_DATA.length) return;
  const d = DRAW_DATA.find(x => x.issue === sel.value);
  box.innerHTML = "";
  if (dateEl) dateEl.textContent = "";
  if (!d) return;
  d.red.forEach(n => box.appendChild(el("span", { class: "chip", text: String(n).padStart(2, "0") })));
  box.appendChild(el("span", { class: "chip blue", text: String(d.blue).padStart(2, "0") }));
  if (dateEl) dateEl.textContent = `开奖日期：${d.date}`;
}
...
async function fetchOnline(opts) {
  opts = opts || {};
  const silent = !!opts.silent; // 静默模式：自动更新用，不弹提示
  const btn = $("btnOnline");
...
function init() {
  ...
  if ($("issueSelect")) { $("issueSelect").onchange = renderIssueResult; refreshIssueSelect(); }
  ...
  // 每次打开页面自动静默更新到最新一期数据（不弹窗，成功后刷新开奖号码与统计）
  setTimeout(() => { fetchOnline({ silent: true }); }, 600);
}
